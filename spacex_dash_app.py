...
launch_dic = [{'label': 'All Sites', 'value': 'ALL'}]
for i in launch_list:
    launch_dic.append({'label':i, 'value':i})
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Rate of Succses X Launch Sites')
        return fig
    else:
        filtered2 = spacex_df[spacex_df['Launch Site'] == entered_site].groupby(['Launch Site', 'class']).size().reset_index(name='class count')
        fig = px.pie(filtered2, values='class count', 
        names='class', 
        title='title')
        return fig
# TASK 4:
